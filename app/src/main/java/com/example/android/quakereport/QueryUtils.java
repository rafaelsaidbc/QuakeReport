...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //esta classe fará o parsing [coleta de informações] da resposta geoJSON do USGS
            //criar a classe baseJsonResponse que armazena as informações citadas acima
            JSONObject baseJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

            //coletar o valor da chave "features"; mapeia o Array JSON com uma lista de tremores;
            // a variável earthquakeArray armazena os dados
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            //faz um loop por todos o Array; iniciamos a contagem com a variável "i" como zero;
            //tentamos se "i" continua válido, se for válido continuamos o loop;
            //no final adicionas o valor "i" em um [sintaxe i++];
            for (int i = 0; i < earthquakeArray.length(); i++) {
                //pegamos o objeto JSON na posição especificada do arranjo e armazena na variável
                //currentEarthquake; começando pela posição zero, representada por "i";
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                //pegamos as informações JSON do objeto "properties" e armazenamos na variável
                //properties;
